        ...
        const bodyText = d.summaryLine
          ...
          : [
              ...
              ...(d.willChainCoreMemory
                ? [
                    "",
                    "系统将**自动顺序执行**《核心记忆》**环节 2：保存并更新现有任务进度**与**环节 3：拆解日报计划为任务表**（环节 2 在自动串联中不写入提取历史、不刷新日报列表；环节 3 不会自动写入任务列表；需在报告提取侧栏补充领导指示后点击「生成任务」）；全部完成后会再推送一条执行摘要。",
                  ]
                : []),
            ].join("\n");
    ...
    const onChainDone = (ev: Event) => {
      ...
      if (d.chainError) {
        setMessages((m) => [
          ...
          {
            role: "assistant",
            text: `**自动串联环节 2/3 异常**：${d.chainError}\n\n可在报告管理 **报告提取** 区手动点击「保存并更新现有任务进度」「拆解日报计划条目」或「生成任务」重试。`,
          },
        ]);
        return;
      }
  ...
  const send = useCallback(async () => {
    ...
    if (awaitingReportBodyFromChatRef.current && !optimizeTargetRef.current) {
      ...
      try {
        ...
        const reply =
          "已根据《核心记忆》将您发送的内容写入**报告管理 → 报告提取 → 日报正文**，并已**自动执行环节 1：解析原文**。随后将**自动顺序执行环节 2（与「保存并更新现有任务进度」相同逻辑，但不保存历史、不刷新日报列表）与环节 3（拆解日报计划为任务表）**；环节 3 不会自动写入任务，请在侧栏补充领导指示后点击「生成任务」。各环节摘要将依次显示在本对话中。";
