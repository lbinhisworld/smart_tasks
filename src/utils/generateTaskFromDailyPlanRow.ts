...
const TASK_CATEGORY_RULES_BLOCK = formatTaskCategoryRulesForLlm();
...
- categoryLevel1（string）：任务大类，必须与下方「任务分类规则」JSON 中某一对象的 level_1 字段**完全一致**（含括号内英文缩写）。
- categoryLevel2（string）：任务子类，必须与所选大类下 level_2 数组中某一元素的 name 字段**完全一致**。
- initiator（string）：可选；客户端会按当前领导视角统一写入交办人，模型可省略或填占位。
- receiverDepartments（string[]）：需配合的部门名称列表，无则输出空数组 []。

---- 任务分类规则（请严格按此层级与名称选类，并结合各子类的 extraction_guide 与正文对齐）----
${TASK_CATEGORY_RULES_BLOCK}`;
...
export async function generateTaskInputFromDailyPlanRow(args: {
  ...
  let categoryLevel1 = pickStr(o, "categoryLevel1", "category_level_1", "任务大类");
  let categoryLevel2 = pickStr(o, "categoryLevel2", "category_level_2", "任务子类");
  if (!categoryLevel1 || !categoryLevel2) {
    const legacy = pickStr(o, "category", "任务类别");
    if (legacy && LEGACY_FLAT_CATEGORY_MAP[legacy]) {
      categoryLevel1 = LEGACY_FLAT_CATEGORY_MAP[legacy].categoryLevel1;
      categoryLevel2 = LEGACY_FLAT_CATEGORY_MAP[legacy].categoryLevel2;
    }
  }
  const { categoryLevel1: c1, categoryLevel2: c2 } = coerceTaskCategoryPair(categoryLevel1, categoryLevel2);
  ...
  const leaderInstructionStored = leaderInstruction.replace(/\s+/g, " ").trim();
  ...
  const out: Omit<Task, "id" | "code" | "createdAt"> = {
    ...
    branch,
    workshop,
    ...(leaderInstructionStored ? { leaderInstruction: leaderInstructionStored } : {}),
